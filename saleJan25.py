...
month_name = st.sidebar.multiselect("Select Month", options = df['Month'].unique(), default = df['Month'].unique()[:5])
st.title(":bar_chart: Sale Dashboard for 2019")
st.markdown('##') # line spacing
total = df['Total'].sum()
no_product = df['Product'].nunique()
...
d, e = st.columns(2)
line_fig_month = px.line(
    bb,
    x = bb.values,
    y = bb.index,
    title = "Sales by Month"
)
d.plotly_chart(line_fig_month, use_container_width=True)

scatter_fig_month = px.scatter(
    df_select,
    x = 'Total',
    y = 'QuantityOrdered',
    title = "Total Sales by Quantity"
)
e.plotly_chart(scatter_fig_month, use_container_width=True)